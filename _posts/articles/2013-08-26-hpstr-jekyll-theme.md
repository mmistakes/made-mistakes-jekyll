...
**Update 10/30/2014**: Grunt is no longer needed to build `main.css`. I've ported all of the LESS stylesheets over to Sass to utilize Jekyll's built-in support for preprocessing .scss files. Updating colors and fonts is as easy as changing a few values in `_sass/_variables.scss` and running `jekyll build` (or deploying to GitHub if you host there).
{: .notice}
